function fun()
{
    ...
    var romanr=document.getElementById("mobile");

    var atpos = email.value.indexOf("@");
    var dotpos = email.value.lastIndexOf(".");

    if(name.value=="" || email.value=="" || romanr.value=="")
    {
    ...
    else if(romanr.value.length!=10)
    {
    ...
    {
        var disp=document.getElementById("abhishek");
    ...
    for (let row of table.rows) 
    {
        ...
        {
        if(cell.innerText == email.value || cell.innerText==romanr.value)
        count=1;
        }
    }
    if(count==1)
